...
const scoreRows = [
  ...
  { label: '창업 형태', description: '신규창업', score: '+0', positive: true },
];
...
const riskItems = [
  {
    title: '1위 동일업종 과밀 경쟁',
    description:
      '선택 상권의 카페 상권은 신규 점포의 고객 확보가 어렵습니다. 차별화 전략이 필요합니다.',
  },
  {
    title: '2위 임차 부담 과중',
    description:
...
const VariableSlider = ({ label }: { label: string }) => (
  <div className="rounded-md bg-[#f7f8fa] px-5 py-4">
    ...
    <div className="relative h-7">
      ...
      <div className="absolute left-[54%] top-0 rounded-sm bg-blue-600 px-2 py-0.5 text-[10px] font-bold text-white">
        보통
      </div>
      ...
      <section className="relative grid w-full max-w-[940px] grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        <section>
          ...
          <div className="rounded-lg bg-white p-5 shadow-sm">
            ...
            <div className="mt-5">
              <h4 className="mb-4 text-xs font-extrabold text-[#191f28]">
                사용자입력
              </h4>
              <div className="grid grid-cols-2 gap-4">
                <label className="block">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-blue-300 px-2 py-0.5 text-[10px] font-bold text-blue-600">
                      공공데이터 연동
                    </span>
                  </div>
                ...
                <label className="block">
                  ...
                  <select defaultValue="업종 선택" className={inputClass}>
                    <option>업종 선택</option>
                    <option>음식점</option>
                    <option>카페/음료</option>
                  </select>
                ...
                <label className="block">
                  <span className={labelClass}>권리금(만)</span>
                  <input
                    type="text"
                    placeholder="전환보증금 기준"
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      className="h-9 rounded-full bg-blue-600 px-3 text-[11px] font-bold text-white"
                    >
                      신규 창업
                    </button>
                    <button
                      type="button"
                      className="h-9 rounded-full border border-[#d8dde5] px-3 text-[11px] font-bold text-[#4e5968]"
                    >
                      양수 창업
                    </button>
                  </div>
            ...
            <div className="mt-5 border-t border-[#e5e8eb] pt-5">
              ...
              <div className="space-y-3">
                {variableFactors.map((factor) => (
                  <VariableSlider key={factor} label={factor} />
                ))}
              </div>
            </div>
        ...
        <section className="rounded-lg bg-white p-5 shadow-sm">
            ...
            <div className="grid grid-cols-[1fr_130px] gap-4">
              ...
              <div className="space-y-3">
                <div className="rounded-sm bg-white/20 px-3 py-2 text-right">
                  ...
                  <p className="mt-1 text-base font-extrabold">40~50%</p>
                </div>
                <div className="rounded-sm bg-white/20 px-3 py-2 text-right">
                  <p className="text-[10px] text-white/80">3년 생존 가능성</p>
                  <p className="mt-1 text-base font-extrabold">20~30%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-md border border-[#e5e8eb] p-4">
            ...
            <div className="space-y-3">
              {riskItems.map((item) => (
                <div key={item.title} className="rounded-md bg-[#f7f8fa] p-3">
